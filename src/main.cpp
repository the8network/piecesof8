...
#if defined(NDEBUG)
# error "8 cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xc39f03cc8dddf26603925ff0004a658be23e013ec416be844b12815c55b5b193");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // 8: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "8 Signed Message:\n";
...
{
    // 8: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 888888); // 8: 888k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 8 * 24 * 60 * 60; // 8: 8 days
static const int64 nTargetSpacing = 8 * 60; // 8: 8 minutes target per block
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // 8: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // 8: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xd778e824ecca35813f170dff5f212066e595561b2306d109f286d1af422dc1eb");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Reuters 20180208 BOJ Kuroda Need to stick with QE for price stability";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 88 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04da08667ed2fb619191ac7bc66463c23a54c7ddaab08aac3703db47ee83afef6a062cd1679e9926323d7991db63107c6b754ba138dd3b6b4535f8ff70bb117790") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1518154862;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 3529157;

        if (fTestNet)
        {
            block.nTime    = 1518154846;
	    block.nNonce   = 1176001;
        }

// Mine genesis - if a new chain is to be created would go here.
        ...
        assert(block.hashMerkleRoot == uint256("0x8e07d2121b072b54eb46efd71fccd7e359c35bf9b294f53c6a4fb7bc60ccbfa8"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc2, 0xba, 0xdd }; // 8: increase each by adding 4 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("8Miner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("8Miner : ProcessBlock, block not accepted");
    }

    return true;
}

void static eightMiner(CWallet *pwallet)
{
    printf("8Miner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("8-miner");
    ...
    try { loop {
        ...
        printf("Running 8Miner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&eightMiner, pwallet));
}
